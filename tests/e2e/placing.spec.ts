...
test("opens in the placing phase with light to move and every piece in hand", async ({ page }) => {
  ...
  await expect(page.getByTestId("turn")).toHaveText(strings.game.toMove.light);

  await expect(inHand(page, "light")).toHaveText("9");
  await expect(inHand(page, "dark")).toHaveText("9");
});

test("places the sides' pieces in turn and counts them out of hand", async ({ page }) => {
  await tap(page, "a1");

  await expect(pointAt(page, "a1")).toHaveAttribute("data-occupant", "light");
  await expect(inHand(page, "light")).toHaveText("8");
  await expect(page.getByTestId("turn")).toHaveText(strings.game.toMove.dark);
...
test("asks for a capture when a mill closes, and holds the turn until it is taken", async ({
  page,
}) => {
