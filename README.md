...
The page is one screen and never scrolls. The board fills the width of a phone and is drawn to fit whatever height is left; everything beside it — teaching, the move list, the summary, the settings — is the panel, which stands next to the board on a screen wide enough for two columns and folds down to a handle on one that is not. Whose turn it is and the question asked before a blunder stay out of it either way, because a question nobody can see is a game that reads as stuck. That is [ADR-0007](docs/adr/0007-the-page-is-one-screen.md), and `tests/e2e/responsive.spec.ts` holds it to it.
