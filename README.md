...
The board takes its size from the screen, clamped between the size at which a point is still a fingertip wide and the room the screen gives it — where the room runs out the page scrolls rather than the board shrinking out from under the finger, and nothing else on the page enters into it. Everything beside it — teaching, the move list, the summary, the settings — is the panel, which stands next to the board on a screen wide enough for two columns and below it on one that is not, where it is reached by scrolling and never folded away. Whose turn it is and the question asked before a blunder stay out of it either way, because a question nobody can see is a game that reads as stuck. That is [ADR-0009](docs/adr/0009-the-board-takes-its-size-from-the-screen.md), which supersedes [ADR-0007](docs/adr/0007-the-page-is-one-screen.md), and `tests/e2e/responsive.spec.ts` holds it to it.
