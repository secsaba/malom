...
**Panel**:
Everything the page shows beside the board: what teaching has to say about the game, the record of it, the Settings, and the game to play next. On a screen with room for both it stands next to the board; on a phone there is not, so it stands below the board and is reached by scrolling. It costs the board nothing either way: the board's size comes from the screen, so what the panel needs is more page rather than less board. Three things are never in it: the board, whose turn it is, and the question a Blunder warning is put back with. The first two are what the game is played on, and the third is a question — one nobody can see is a game that reads as stuck.
_Hungarian_: none — nothing in the interface names the panel; each block standing in it is shown under its own name
_Avoid_: sidebar, drawer, tray, bottom sheet — the last two naming a shape it does not take; and _analysis panel_, which names half of what stands in it
